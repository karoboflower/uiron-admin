...
```json
{
  "scripts": {
    ...
    "build:naive": "pnpm run build --filter=@uiron/naive",
...
Take the addition of the test environment `test` to `@uiron/web-antd` as an example:
...
"scripts": {
    ...
    "build:analyze": "turbo build:analyze",
    "build:antd": "pnpm run build --filter=@uiron/web-antd",
    "build-test:antd": "pnpm run build --filter=@uiron/web-antd build:test",
...
"tasks": {
    ...
    "build-test:antd": {
      "dependsOn": ["@uiron/web-antd#build:test"],
      "outputs": ["dist/**"]
    },

    "@uiron/web-antd#build:test": {
      "dependsOn": ["^build"],
      "outputs": ["dist/**"]
    },
